    ...
    for player_index, player in enumerate(players):
        if player.dead == False:
            ...
            if player.respawn_timer < 0:
                player.dead = False
                (player.angle, player.angular_speed, player.angular_acceleration) = (
                    0,
                    0,
                    0,
                )
                (player.x_position, player.x_speed, player.x_acceleration) = (400, 0, 0)
                (player.y_position, player.y_speed, player.y_acceleration) = (400, 0, 0)
        ...
        screen.blit(
            target_sprite,
            (
                targets[player.target_counter][0] - int(target_sprite.get_width() / 2),
