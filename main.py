...
for i in range(1, 8):
    ...
    image.convert()
    target_animation.append(pygame.transform.scale(
    ...
    for player_index, player in enumerate(players):
        if player.dead == False:
            ...
            thruster_left, thruster_right = player.act(
                [targets[player.target_counter][0] - player.x_position, player.x_speed, targets[player.target_counter][1] - player.y_position, player.y_speed, player.angle, player.angular_speed])

            # Calculate accelerations according to Newton's laws of motion
            player.x_acceleration += -(thruster_left + thruster_right) * \
                sin(player.angle*pi/180)/mass
            player.y_acceleration += -(thruster_left + thruster_right) * \
            ...
            dist = sqrt((player.x_position - targets[player.target_counter][0])**2 +
            ...
            if player.respawn_timer < 0:
                player.dead = False
                (player.angle, player.angular_speed,
                 player.angular_acceleration) = (0, 0, 0)
                (player.x_position, player.x_speed,
                 player.x_acceleration) = (400, 0, 0)
                (player.y_position, player.y_speed,
